...
int GRIMPL_NS::read_cloudy_HydrogenFractionByMass(const char* fname,
                                                   double* hfrac)
{
  const char* attr_name = "HydrogenFractionByMass";
  ...
  if (exists == 0) {
    ...
    hid_t attr_id = H5Aopen_name(file_id, attr_name);
    if (attr_id < 0) {
      std::fprintf(stderr, "Failed to open \"%s\" attribute of %s.\n",
                   attr_name, fname);
    } else {
      // H5Aread writes one value per element of the attribute's dataspace,
      // so insist on a single element before reading into *hfrac
      hid_t space_id = H5Aget_space(attr_id);
      hssize_t npoints = (space_id < 0) ? -1
      ...
      if (npoints != 1) {
        std::fprintf(stderr,
                     "\"%s\" attribute of %s must hold a single value.\n",
                     attr_name, fname);
      } else if (H5Aread(attr_id, H5T_NATIVE_DOUBLE, &tmp) < 0) {
        std::fprintf(stderr, "Failed to read \"%s\" attribute of %s.\n",
                     attr_name, fname);
      } else if (!(tmp > 0.0 && tmp <= 1.0)) {  // also rejects NaN
        std::fprintf(stderr,
                     "\"%s\" attribute of %s is %g; it must lie in (0, 1].\n",
                     attr_name, fname, tmp);
      } else {
        *hfrac = tmp;
        out = 1;
    ...
    std::fprintf(stderr, "Failed to check for \"%s\" attribute of %s.\n",
                 attr_name, fname);
  }
  H5Fclose(file_id);
  return out;
