...
.. c:var:: int UVbackground
   ...
   photo-ionization rates of C and O are also read from the UV background
   table (datasets ``kphCI`` and ``kphOI``); if the table does not contain
   them, these rates are set to zero and a warning is printed.
   Default: 0.
